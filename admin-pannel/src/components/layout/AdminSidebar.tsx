...
export function AdminSidebar() {
  ...
  return (
    <div className="flex h-full flex-col bg-slate-50 border-r border-slate-300 transition-colors duration-200">
      {/* Logo/Brand */}
      <div className="flex h-20 items-center px-6 border-b border-slate-300/50 bg-slate-50/50 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center shadow-lg shadow-indigo-900/20 rotate-3 hover:rotate-0 transition-transform duration-500">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-[10px] uppercase tracking-[0.3em] font-bold text-slate-600">Admin Suite</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-8 overflow-y-auto scrollbar-thin scrollbar-thumb-navy-800 scrollbar-track-transparent space-y-10">
        {navigationGoups.map((group) => (
          <div key={group.title} className="space-y-4">
            <h3 className="px-3 text-[10px] font-black uppercase tracking-[0.25em] text-slate-700/80">
              {group.title}
            </h3>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        ...
                        isActive
                          ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-900/20 ring-1 ring-white/10'
                          : 'text-slate-600/90 hover:text-indigo-600 hover:bg-slate-100/80'
                      )
                    }
                  >
                    <Icon
      ...
      <div className="p-5 border-t border-slate-300 bg-slate-50/80">
        <div className="flex items-center gap-3 p-3 rounded-2xl border border-white/5 bg-white/40 hover:bg-white/60 transition-all duration-300 group cursor-pointer group shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5">
               ...
               <p className="text-[10px] text-slate-600 font-black uppercase tracking-widest">{user?.role}</p>
            </div>
          </div>
          <Button
            variant="ghost"
            ...
            className="h-9 w-9 text-slate-700 hover:text-rose-600 hover:bg-rose-500/10 transition-colors rounded-xl"
          >
