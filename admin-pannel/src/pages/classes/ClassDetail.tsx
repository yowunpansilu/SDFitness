  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-slate-400 font-black uppercase text-xs tracking-widest animate-pulse">Accessing Encrypted Protocol...</div>
      </div>
    );
  }
  ...
  if (!gymClass) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <div className="text-rose-500 font-black uppercase text-xs tracking-widest">Protocol Not Found</div>
        <Button onClick={() => navigate('/classes')} variant="outline" className="rounded-xl font-bold uppercase text-[10px] tracking-widest px-6">Return to Grid</Button>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid gap-6 md:grid-cols-4">
        {[
          ...
          { label: 'Price Metric', value: `$${gymClass.price || 0}`, sub: 'Per Activation', color: 'text-rose-500', bg: 'bg-rose-500/10' }
        ].map((stat, i) => (
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="bg-white dark:bg-navy-900 border-slate-200 dark:border-navy-800 rounded-[2.5rem] shadow-sm transition-colors overflow-hidden">
            <CardHeader className="p-10 pb-4">
              <CardTitle className="text-sm font-black uppercase tracking-[0.25em] text-slate-400 dark:text-navy-600 flex items-center gap-3">
            ...
            <CardContent className="p-10 pt-0 space-y-10">
              <div className="p-8 rounded-3xl bg-slate-50 dark:bg-navy-950/50 border border-slate-100 dark:border-navy-800 transition-colors">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-navy-600 mb-3 block">Manifesto</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-navy-600">Faculty Assignment</label>
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-navy-600 flex items-center gap-2">
                    <MapPin className="h-4 w-4 text-rose-500" />
                    Sector Location
                  </label>
                  <p className="text-2xl font-black tracking-tight text-slate-900 dark:text-white transition-colors">{gymClass.location || 'UNASSIGNED SECTOR'}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  ...
                  <p className="text-lg font-bold text-slate-900 dark:text-white transition-colors">
                    {gymClass.schedule?.dayOfWeek?.toUpperCase() || 'UNSCHEDULED'} <span className="text-indigo-600 dark:text-indigo-400 ml-2 font-black">{gymClass.schedule?.startTime}</span>
                  </p>
                </div>

                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-navy-600">Initialization</label>
                  <p className="text-lg font-bold text-slate-900 dark:text-white transition-colors">
              ...
              <div className="divide-y divide-slate-50 dark:divide-navy-950">
                {(gymClass.enrolledMembers || []).map((member: any) => (
                  ...
                  >
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="text-lg font-black text-slate-900 dark:text-white transition-colors group-hover:text-indigo-600 dark:group-hover:text-indigo-400 leading-tight">
                ...
                {(gymClass.enrolledMembers || []).length === 0 && (
                  <div className="p-20 text-center">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 dark:text-navy-800">No Active Deployments Detected</p>
                  </div>
                )}
              </div>
            </CardContent>
        ...
        <div className="space-y-8">
          <Card className="bg-white dark:bg-navy-900 border-slate-200 dark:border-navy-800 rounded-[2.5rem] shadow-sm transition-colors overflow-hidden">
            <CardHeader className="p-8 pb-4">
              <CardTitle className="text-sm font-black uppercase tracking-[0.25em] text-slate-400 dark:text-navy-600 flex items-center gap-3 font-black">
              ...
              <div className="text-center space-y-6">
                <div className="relative inline-block">
                  <Avatar className="h-32 w-32 border-4 border-slate-50 dark:border-navy-950 shadow-2xl transition-transform hover:scale-105">
                    ...
                    <AvatarFallback className="bg-gradient-to-br from-indigo-500 to-indigo-700 text-white text-3xl font-black">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-navy-600 mt-1">{gymClass.trainer?.userId?.email || 'OFFLINE'}</p>
                </div>

                <div className="pt-4 border-t border-slate-50 dark:border-navy-950">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-navy-600 block mb-4">Core Matrices</label>
                  <div className="flex flex-wrap gap-2 justify-center">
                    ...
                    {(gymClass.trainer?.specialization || []).length === 0 && (
                      <span className="text-[9px] font-black uppercase text-slate-300 dark:text-navy-800 tracking-widest">Generalist Matrix</span>
                    )}
                  </div>
                </div>
          ...
          <Card className="bg-slate-50 dark:bg-navy-950/50 border-none rounded-[2.5rem] transition-colors overflow-hidden">
            <CardHeader className="p-8 pb-4">
              <CardTitle className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-navy-700">Quick Protocols</CardTitle>
            </CardHeader>
