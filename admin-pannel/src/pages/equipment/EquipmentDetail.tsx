  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-slate-400 font-bold uppercase text-xs tracking-widest animate-pulse">Syncing Asset Data...</div>
      </div>
    );
  }
  ...
  if (!equipment) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <div className="text-rose-500 font-bold uppercase text-xs tracking-widest">Asset Not Found</div>
        <Button onClick={() => navigate('/equipment')} variant="outline" className="rounded-xl font-bold uppercase text-xs tracking-widest px-6">Return to Inventory</Button>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              <Badge className={cn('font-bold text-xs uppercase tracking-widest rounded-lg border shadow-sm px-2 py-1', (statusConfig as any)[equipment.status]?.color || 'bg-slate-500/20 text-slate-400 border-slate-500/30')}>
                {(statusConfig as any)[equipment.status]?.label || equipment.status}
              </Badge>
            ...
            <p className="text-slate-500 dark:text-navy-400 font-medium mt-1">
              Asset ID: <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded-md">{equipment.serialNumber}</span> • {equipment.brand} {equipment.model}
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-4">
        <Card className="bg-white dark:bg-navy-900 border-slate-200/60 dark:border-navy-800 shadow-sm rounded-3xl overflow-hidden group transition-all duration-500 hover:shadow-2xl hover:shadow-indigo-500/5">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            ...
            <div className={cn("p-2 rounded-xl transition-transform group-hover:scale-110", (statusConfig as any)[equipment.status]?.color || 'bg-slate-500/20')}>
              <Package className="h-4 w-4" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-slate-900 dark:text-white uppercase">{(statusConfig as any)[equipment.status]?.label || equipment.status}</div>
            ...
            <CardContent className="p-10 pt-0 space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-2">
                  ...
                  <div className="flex">
                    <Badge className={cn('px-3 py-1 font-bold text-xs uppercase tracking-widest rounded-xl border-none shadow-sm', (categoryColors as any)[equipment.category] || 'bg-slate-500/20 text-slate-400 border-slate-500/30')}>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight">{equipment.location}</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight">
                    {equipment.warrantyExpiry ? `Expires ${new Date(equipment.warrantyExpiry).toLocaleDateString(undefined, {year: 'numeric', month: 'long', day: 'numeric'})}` : 'No Warranty Recorded'}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                {maintenanceHistory.length === 0 && (
                  <div className="p-12 text-center">
                    <p className="text-xs font-bold uppercase tracking-widest text-slate-300 dark:text-navy-700">No lifecycle events recorded for this asset.</p>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          <Card className="bg-indigo-600 dark:bg-indigo-600 border-none rounded-[2.5rem] overflow-hidden shadow-2xl shadow-indigo-500/20 group relative">
            ...
            <CardContent className="p-8 pt-0 relative z-10 space-y-8">
              <div className="space-y-2">
                ...
                <p className="text-xl font-bold text-white">{equipment.lastMaintenanceDate ? new Date(equipment.lastMaintenanceDate).toLocaleDateString(undefined, {month: 'long', day: 'numeric'}) : 'N/A'}</p>
              </div>
              <div className="space-y-2">
                ...
                <p className={cn('text-3xl font-bold text-white tracking-tight', daysUntilMaintenance && daysUntilMaintenance < 7 ? 'text-amber-300' : '')}>
                  {equipment.nextMaintenanceDate ? new Date(equipment.nextMaintenanceDate).toLocaleDateString(undefined, {month: 'long', day: 'numeric'}) : 'STANDBY'}
                </p>
