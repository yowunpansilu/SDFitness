    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-navy-400 font-bold uppercase tracking-widest text-xs">Scanning Hardware Matrix...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-white">
            Hardware <span className="text-indigo-400 italic font-medium">Inventory</span>
          </h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="bg-navy-900 border-navy-800 shadow-sm rounded-3xl overflow-hidden group transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-[10px] font-black uppercase tracking-widest text-navy-500">Total Assets</CardTitle>
            <div className="p-2 rounded-xl bg-indigo-500/10 text-indigo-400 transition-transform group-hover:scale-110 shadow-sm">
              <Shield className="h-4 w-4" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-white">{equipmentArray.length}</div>
            <p className="text-[9px] font-black text-navy-600 mt-1 uppercase tracking-widest">Active nodes</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-navy-900 border-navy-800 shadow-sm rounded-3xl overflow-hidden group transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            ...
            <div className="p-2 rounded-xl bg-emerald-500/10 text-emerald-400 transition-transform group-hover:scale-110 shadow-sm">
          ...
          <CardContent>
            ...
            <p className="text-[9px] font-black text-navy-600 mt-1 uppercase tracking-widest">Fully optimized</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-navy-900 border-navy-800 shadow-sm rounded-3xl overflow-hidden group transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            ...
            <div className="p-2 rounded-xl bg-amber-500/10 text-amber-400 transition-transform group-hover:scale-110 shadow-sm">
              <Wrench className="h-4 w-4" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-white">{maintenanceCount}</div>
            <p className="text-[9px] font-black text-navy-600 mt-1 uppercase tracking-widest">Under protocol review</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-navy-900 border-navy-800 shadow-sm rounded-3xl overflow-hidden group transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            ...
            <div className="p-2 rounded-xl bg-rose-500/10 text-rose-400 transition-transform group-hover:scale-110 shadow-sm">
              <AlertCircle className="h-4 w-4" />
            </div>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-white">{criticalCount}</div>
            <p className="text-[9px] font-black text-navy-600 mt-1 uppercase tracking-widest">Immediate response needed</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-navy-500 group-focus-within:text-indigo-400 transition-all" />
              <input
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredEquipment.map((item) => {
          ...
          return (
            <Card
              key={item._id}
              className="bg-navy-900 border-navy-800 shadow-sm hover:shadow-2xl hover:shadow-indigo-500/5 hover:-translate-y-2 transition-all duration-500 rounded-[2.5rem] overflow-hidden group border-2 border-transparent hover:border-indigo-500/20"
            >
              <CardContent className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-black uppercase text-navy-600 mt-1 tracking-widest flex items-center gap-2">
                ...
                {/* Condition Bar */}
                <div className="space-y-2 bg-navy-950/50 p-4 rounded-3xl transition-all">
                  <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest">
                    <span className="text-navy-500">Node Integrity</span>
                ...
                <div className="flex gap-2 pt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <Button variant="ghost" className="flex-1 h-10 rounded-xl bg-navy-950 text-indigo-400 hover:bg-indigo-500 hover:text-white font-black text-[9px] uppercase tracking-widest active:scale-95 transition-all">
                      Diagnostics
                    </Button>
                    <Button 
                      onClick={() => handleDelete(item._id, item.name)}
                      variant="ghost" 
                      className="h-10 w-10 p-0 rounded-xl bg-navy-950 text-rose-500 hover:bg-rose-500 hover:text-white font-black active:scale-95 transition-all"
                    >
      ...
      {filteredEquipment.length === 0 && (
         <div className="text-center py-24 bg-navy-900 rounded-[3rem] border-2 border-dashed border-navy-800">
            <Search className="h-12 w-12 text-navy-800 mx-auto mb-6" />
            <h3 className="text-white font-black text-xl uppercase tracking-widest">Hardware Node Not Found</h3>
            <p className="text-navy-500 font-bold text-xs uppercase tracking-widest">Adjustment of tracking parameters required</p>
         </div>
      )}
    </div>
  );
