      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100 dark:border-navy-800">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl font-black italic tracking-tight text-slate-900 dark:text-white transition-colors">
              FINANCIAL <span className="text-indigo-600 dark:text-indigo-400 italic">INITIALIZATION</span>
            </h1>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 dark:text-navy-600 mt-1">Deploy New Transaction Layer</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            onClick={() => navigate('/payments')}
            className="h-12 px-8 rounded-2xl font-black uppercase text-[10px] tracking-widest text-slate-400 dark:text-navy-500 hover:text-rose-500 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all"
          >
          ...
          <Card className="bg-white dark:bg-navy-900 border-slate-200 dark:border-navy-800 rounded-[2.5rem] shadow-sm transition-colors overflow-hidden group">
            ...
            <CardContent className="p-10 pt-0 space-y-8">
              <div className="relative">
                <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic mb-3 block">
                ...
                <div className="relative group/search">
                  ...
                  <Input
                    ...
                    className="h-14 bg-slate-50 dark:bg-navy-950/50 border-slate-100 dark:border-navy-800 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold placeholder:text-slate-300 dark:placeholder:text-navy-800 pl-12 uppercase italic tracking-tight"
                ...
                {showMemberList && memberSearch && (
                  <div className="absolute z-50 w-full mt-4 bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-800 rounded-3xl shadow-2xl shadow-indigo-500/10 max-h-80 overflow-y-auto animate-in fade-in slide-in-from-top-4 duration-300">
                    {filteredMembers.length > 0 ? (
                      filteredMembers.map((member) => (
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              <p className="text-lg font-black text-slate-900 dark:text-white uppercase italic group-hover/item:text-indigo-600 dark:group-hover/item:text-indigo-400 transition-colors">{member.name}</p>
                              <p className="text-[10px] font-bold text-slate-400 dark:text-navy-500 uppercase tracking-widest mt-0.5">{member.email}</p>
                            </div>
                            <Badge variant="outline" className="font-black text-[9px] uppercase tracking-widest border-indigo-100 dark:border-navy-800 text-indigo-500">
                              {member.plan}
                            </Badge>
                          </div>
                        </div>
                      ...
                      <div className="p-10 text-center text-slate-400 dark:text-navy-700 italic font-bold uppercase text-[10px] tracking-widest">No matching nodes found</div>
                    )}
                  </div>
                )}
                ...
                <div className="p-8 rounded-[2rem] bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 relative animate-in zoom-in-95 duration-500">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="text-xl font-black text-slate-900 dark:text-white uppercase italic">{selectedMember.name}</p>
                        <p className="text-[10px] font-bold text-slate-500 dark:text-navy-400 uppercase tracking-widest">{selectedMember.email}</p>
            ...
            <CardContent className="p-10 pt-0 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <Label htmlFor="paymentType" className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic">
                    Protocol Category <span className="text-rose-500">*</span>
                  </Label>
                  <Select onValueChange={(value) => setValue('paymentType', value as any)}>
                    <SelectTrigger className="h-14 bg-slate-50 dark:bg-navy-950/50 border-slate-100 dark:border-navy-800 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold uppercase italic">
                ...
                {paymentType === 'membership' && (
                  <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic">Target Plan</Label>
                ...
                <div className="space-y-3">
                  <Label htmlFor="amount" className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic">
                  ...
                  <div className="relative group/amount">
                    ...
                    <Input
                      ...
                      className="h-14 bg-slate-50 dark:bg-navy-950/50 border-slate-100 dark:border-navy-800 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all font-black text-xl italic pl-12"
                      placeholder="0.00"
                    />
                  </div>
                  {errors.amount && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                {paymentMethod === 'card' && (
                  <>
                    <div className="space-y-3 animate-in slide-in-from-left-4 duration-500">
                      <Label htmlFor="cardBrand" className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic">Card Fleet</Label>
                      <Select onValueChange={(value) => setValue('cardBrand', value)}>
                        <SelectTrigger className="h-14 bg-slate-50 dark:bg-navy-950/50 border-slate-100 dark:border-navy-800 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold uppercase italic font-black">
                    ...
                    <div className="space-y-3 animate-in slide-in-from-right-4 duration-500">
                      <Label htmlFor="lastFourDigits" className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic">Terminal Suffix</Label>
                      <Input
                        id="lastFourDigits"
                        {...register('lastFourDigits')}
                        className="h-14 bg-slate-50 dark:bg-navy-950/50 border-slate-100 dark:border-navy-800 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all font-black text-center tracking-[0.5em]"
                        placeholder="••••"
                        maxLength={4}
                      />
                    </div>
                ...
                {paymentMethod === 'bank-transfer' && (
                  <div className="md:col-span-2 space-y-3 animate-in zoom-in-95 duration-500">
                    <Label htmlFor="referenceNumber" className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic">Operational Reference</Label>
                    <Input
                      id="referenceNumber"
                      {...register('referenceNumber')}
                      className="h-14 bg-slate-50 dark:bg-navy-950/50 border-slate-100 dark:border-navy-800 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all font-mono font-black"
                ...
                <div className="md:col-span-2 space-y-3">
                  <Label htmlFor="transactionId" className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 italic">Transaction Identifier</Label>
                  <Input
                    id="transactionId"
                    {...register('transactionId')}
                    className="h-14 bg-slate-50 dark:bg-navy-950/50 border-slate-100 dark:border-navy-800 text-slate-900 dark:text-white rounded-2xl focus:ring-4 focus:ring-indigo-500/10 transition-all font-mono font-black placeholder:opacity-30"
        ...
        <div className="lg:col-span-1 space-y-10">
          <Card className="bg-white dark:bg-navy-900 border-slate-200 dark:border-navy-800 rounded-[2.5rem] shadow-xl shadow-indigo-500/5 sticky top-10 overflow-hidden ring-1 ring-slate-100 dark:ring-navy-800">
            <CardHeader className="bg-slate-50/50 dark:bg-navy-950/50 p-8 pb-4 border-b border-slate-50 dark:border-navy-800">
              <CardTitle className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic">LEDGER PREVIEW</CardTitle>
              <CardDescription className="text-[10px] font-bold text-slate-400 dark:text-navy-600 uppercase tracking-widest mt-1">Audit before final commitment</CardDescription>
            </CardHeader>
            <CardContent className="p-8 space-y-8 font-medium">
              <div className="space-y-4">
                <div className="flex justify-between text-[11px] font-black uppercase tracking-tight">
                  <span className="text-slate-400 dark:text-navy-600">Base Metric</span>
                  <span className="text-slate-900 dark:text-white">${amount.toFixed(2)}</span>
                </div>

                {discountType && discountType !== 'none' && (
                  <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-tight p-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20">
                ...
                <div className="pt-2">
                  <div className="flex justify-between items-baseline mb-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-navy-600">Total Net Value</span>
                  ...
                  <p className="text-[9px] font-bold text-slate-300 dark:text-navy-800 text-right uppercase tracking-widest italic font-black">Values synced with global matrix</p>
                </div>
              </div>

              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-navy-700 italic border-b border-slate-50 dark:border-navy-800 pb-2">Transmission Protocols</h4>
                <div className="space-y-4">
                  <div className="flex items-center space-x-4 group cursor-pointer" onClick={() => setValue('sendReceipt', !sendReceipt)}>
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 group-hover:text-indigo-600 transition-colors">Electronic Receipt Pulse</span>
                  </div>
                  <div className="flex items-center space-x-4 group cursor-pointer" onClick={() => setValue('sendSMS', !sendSMS)}>
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-navy-500 group-hover:text-indigo-600 transition-colors">Tactical SMS Alert</span>
                  </div>
                </div>
              </div>
              ...
              {selectedMember && (
                <div className="p-5 rounded-2xl bg-slate-50 dark:bg-navy-950/50 border border-slate-50 dark:border-navy-800 transition-all hover:bg-white dark:hover:bg-navy-800 ring-1 ring-transparent hover:ring-indigo-500/20">
                  <p className="text-[8px] font-black text-slate-400 dark:text-navy-700 uppercase mb-2 tracking-widest italic">Authorized For</p>
                  <p className="text-sm font-black text-slate-900 dark:text-white uppercase truncate italic">{selectedMember.name}</p>
                </div>
              )}

              <Button
                onClick={handleSubmit(onSubmit)}
                className="w-full h-14 bg-slate-900 dark:bg-indigo-600 hover:bg-black dark:hover:bg-indigo-700 text-white font-black uppercase text-[11px] tracking-[0.2em] rounded-2xl shadow-2xl shadow-indigo-500/20 transition-all hover:scale-[1.02] active:scale-[0.98] mt-4"
              >
                COMMIT TO LEDGER
              </Button>
            </CardContent>
          ...
          <div className="text-center">
             <p className="text-[8px] font-black uppercase tracking-[0.4em] text-slate-300 dark:text-navy-800 italic">Authorized System Personnel Only</p>
          </div>
        </div>
      </form>
    </div>
