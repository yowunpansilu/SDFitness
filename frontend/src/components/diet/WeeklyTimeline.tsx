...
export function WeeklyTimeline({ days, onDaySelect, activeDay }: WeeklyTimelineProps) {
    return (
        <div className="relative group">
            <div className="flex gap-4 overflow-x-auto pb-4 pt-2 px-1 scrollbar-hide no-scrollbar">
                {days.map((day, i) => (
                    <button
                        key={i}
                        onClick={() => onDaySelect(i)}
                        className={cn(
                            "flex flex-col items-center justify-between min-w-[100px] h-[130px] rounded-2xl border transition-all duration-300 transform hover:-translate-y-1",
                            activeDay === i 
                        ...
                        <div className="pb-4 relative">
                            <svg className="w-12 h-12 transform -rotate-90">
                                ...
                                <circle
                                    ...
                                    cy="24"
                                    r="18"
                                    stroke={activeDay === i ? "#F59E0B" : "#F59E0B"}
                                    strokeWidth="4"
                                    fill="transparent"
                                    strokeDasharray={113}
                                    strokeDashoffset={113 - (113 * day.progress)}
                                    strokeLinecap="round"
                                />
                            </svg>
                            {/* Nested dots for different macros as seen in screenshot */}
                            <div className="absolute inset-0 flex items-center justify-center">
                                <div className={cn(
                                    "w-1.5 h-1.5 rounded-full scale-75",
                                    activeDay === i ? "bg-white" : "bg-primary-200"
                                )} />
                            </div>
                        </div>
                    </button>
            ...
            <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="bg-white rounded-full shadow-lg p-2 border border-primary-50">
