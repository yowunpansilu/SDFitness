...
export function ProgressChart({ logs, onRefresh }: { logs: WeightLog[], onRefresh: () => void }) {
    ...
    const chartData = useMemo(() => {
        ...
        for (let i = 0; i < filteredLogs.length; i++) {
            ...
            let trendSum = currentObj.weight.value;
            let trendDivisor = 1;
            
            if (i > 0) { trendSum += filteredLogs[i-1].weight.value; trendDivisor++; }
            if (i > 1) { trendSum += filteredLogs[i-2].weight.value; trendDivisor++; }
            
            result.push({
                fullDate: currentObj.date,
                date: new Date(currentObj.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }),
                weight: currentObj.weight.value,
