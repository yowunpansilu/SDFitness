...
export function WeightInsights({ logs }: { logs: WeightLog[] }) {
    ...
    const initWeight = member?.initialWeight?.value || 0;

    // Calculations
    const totalDiff = currentWeight - initWeight;
    const isLosingTarget = targetWeight < initWeight; 
    ...
    if (sortedLogs.length >= 2) {
        ...
        if (daysDiff > 0) {
            const weightDiff = newestLog.weight.value - oldestLog.weight.value;
